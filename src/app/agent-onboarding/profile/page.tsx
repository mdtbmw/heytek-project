...
import { ArrowRight, UserCircle, CalendarDays, Camera, Sparkles, LogIn } from 'lucide-react';
import { DatePicker } from '@/components/ui/date-picker';
import Link from 'next/link';
// Removed useToast as it's not directly used here for email checks anymore

export default function ProfilePage() {
  const { onboardingData, updateOnboardingData, setCurrentStep } = useAgentOnboarding();
  const router = useRouter();
  // const { toast } = useToast(); // Not needed here anymore
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="flex justify-between items-center pt-4">
                <Button variant="link" asChild className="text-muted-foreground hover:text-primary p-0 h-auto no-blue-link">
                    <Link href="/login">
                        <LogIn className="mr-2 h-4 w-4"/> Already on HeyTek? Log In
                    </Link>
                </Button>
                <Button type="submit" className="bg-primary hover:bg-primary/80 text-primary-foreground dark:text-primary-foreground text-lg py-6 rounded-md shadow-md hover:shadow-lg transition-shadow">
